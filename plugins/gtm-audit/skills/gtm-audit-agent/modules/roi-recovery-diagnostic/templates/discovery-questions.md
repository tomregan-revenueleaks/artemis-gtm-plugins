...
Then run the math (playbook Steps 2-6): hand the metrics to the benchmark-analyst sub-agent and the recoverable computation to the roi-quantifier sub-agent (read each `modules/roi-recovery-diagnostic/agents/<name>.md` file and spawn a subagent with its contents as the prompt), and assemble the roi-report-template.
