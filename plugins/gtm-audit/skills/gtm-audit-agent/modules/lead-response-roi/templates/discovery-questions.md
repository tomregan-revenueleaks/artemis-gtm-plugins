...
Then run Diagnose (playbook Step 4): map the tier to its multiplier, back-solve the ceiling (`min(1, enteredRate ÷ multiplier)`), compute `revenueLostPerYear = (ceiling − current) × leads × deal × 12`, and assemble the leak-report-template. You can hand the placement to the benchmark-analyst sub-agent and the dollar math to the leak-quantifier sub-agent (read each `modules/lead-response-roi/agents/<name>.md` file and spawn a subagent with its contents as the prompt).
