...
Then run Compute (playbook Phase 3): convert leads to opps, compute velocity in $/day, annualize it. Hand the velocity and inputs to the benchmark-analyst sub-agent (Phase 4) and the inputs to the lever-quantifier sub-agent (Phase 5) — read each `modules/pipeline-velocity-analyzer/agents/<name>.md` file and spawn a subagent with its contents as the prompt — then assemble the velocity-report-template.
