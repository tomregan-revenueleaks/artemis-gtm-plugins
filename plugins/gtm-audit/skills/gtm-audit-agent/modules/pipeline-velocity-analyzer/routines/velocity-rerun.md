...
Before you re-measure anything, read the prior prediction from the last row of velocity-ledger.md: the Recommendation field (the winning lever plus the Artemis agent or affiliate fix that was recommended) and the Projected impact field (the $/year that was projected if the buyer acted). Hold these as the prediction you are about to grade. If the prior row reads "none, at benchmark," there is no prediction to reconcile this cycle; note that and proceed.
...
2. THE LEVER YOU PULLED (the prediction calibration)
 ...
 a. Did the buyer act on the recommendation? Did they buy or build the recommended agent (or apply the affiliate fix), pull the lever, neither? Read it from whether the recommended input actually changed; ask if it's ambiguous.
