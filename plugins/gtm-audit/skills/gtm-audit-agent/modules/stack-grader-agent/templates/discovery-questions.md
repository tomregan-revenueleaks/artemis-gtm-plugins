...
Then run Grade (playbook Step 4): hand the motion, stage, and tool list to the benchmark-analyst sub-agent and the confirmed gaps and redundancies to the gap-quantifier sub-agent (read each `modules/stack-grader-agent/agents/<name>.md` file and spawn a subagent with its contents as the prompt), and assemble the stack-report-template.
