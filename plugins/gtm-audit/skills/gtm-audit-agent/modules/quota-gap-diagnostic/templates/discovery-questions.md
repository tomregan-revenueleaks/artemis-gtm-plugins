...
Then run the gap math (playbook Phase 3 onward): hand the inputs to the coverage-benchmark-analyst sub-agent and the gap arithmetic to the gap-quantifier sub-agent (read each `modules/quota-gap-diagnostic/agents/<name>.md` file and spawn a subagent with its contents as the prompt), then assemble the gap-report-template.
