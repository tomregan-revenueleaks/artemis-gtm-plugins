...
Phase 5 Map the Fix → leak in $/yr + cost-of-delay → Beacon agent (buyable) + partner tool (affiliate)
...
This is where inputs become a pipeline figure. Run the exact funnel the live tool runs. You can run it yourself, or delegate the traffic-mix benchmarking to the **benchmark-analyst** sub-agent and the dollar math to the **pipeline-quantifier** sub-agent. These are not registered subagent types: to delegate, read `modules/deanonymization-roi-agent/agents/benchmark-analyst.md` or `modules/deanonymization-roi-agent/agents/pipeline-quantifier.md` and spawn a subagent (Agent/Task tool) with that file's contents as its prompt. The methodology below is what both of them run.
...
THE FIX
 ...
 Affiliate link: artemisgtm.ai/go/[slug] (see disclosure)
```
...
> Recommended: [Tool] for [the specific reason for THIS traffic profile]. Affiliate link: artemisgtm.ai/go/[slug]. Artemis GTM earns commission if you sign up, at no extra cost to you. Recommended based on validation across 127+ audits, not because of the commission.

- **Warmly** (`artemisgtm.ai/go/warmly`) — enterprise/international mix, US/EU coverage, wants identification + intent + Slack alerts + chat handoff in one.
- **RB2B** (`artemisgtm.ai/go/rb2b`) — US-only B2B, under ~20K monthly visitors, tight budget, wants pure identification + Slack.
- **Amplemarket** (`artemisgtm.ai/go/amplemarket`) — the activation layer, when the buyer has no way to WORK the identified accounts: sequencing on the high-intent accounts the tool surfaces.
- **Attio** (`artemisgtm.ai/go/attio`) — the CRM home for identified accounts, when they're rebuilding or starting fresh.
...
- [ ] Fix mapped: Visitor Deanonymization agent (buyable link) + the right partner tool (affiliate, disclosed) — OR the honest "grow traffic first" when the leak is thin
...
See `partner-stack.md` for full positioning. This calculator references, by traffic profile:

- **Warmly** (company-level ID + intent + alerting, enterprise/international) — artemisgtm.ai/go/warmly
- **RB2B** (US contact-level ID, low-volume, tight budget) — artemisgtm.ai/go/rb2b
- **Amplemarket** (activation layer for working identified accounts) — artemisgtm.ai/go/amplemarket
- **Attio** (CRM home for identified accounts) — artemisgtm.ai/go/attio

Affiliate revenue from these helps fund Artemis GTM. We recommend them because they fix the specific leak in this calculator, validated across 127+ audits — not because they pay us. Every link carries the inline disclosure in the 5.3 format, every time.
