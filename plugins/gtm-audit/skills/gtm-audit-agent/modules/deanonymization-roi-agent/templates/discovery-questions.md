...
Then run Quantify (playbook Phase 3): hand the traffic profile to the benchmark-analyst sub-agent to set the identification rate and ICP-fit-adjusted visitor count, and the result to the pipeline-quantifier sub-agent for the dollar math (read each `modules/deanonymization-roi-agent/agents/<name>.md` file and spawn a subagent with its contents as the prompt), then assemble the leak-report-template.
