...
This Skill Package includes recommendations for tools from Artemis GTM partner companies (Warmly, RB2B, Amplemarket, Apollo, Attio, Sybill, Attention, Maildoso). When you click through and sign up via the in-skill referral links (`artemisgtm.ai/go/[partner]`), Artemis GTM may earn a commission, at no extra cost to you.
...
The build modules this audit routes to are Artemis GTM's own products, not affiliate links; they're the paid modules that implement the fixes this audit finds. The audit names and routes them, it never sells them inline.
