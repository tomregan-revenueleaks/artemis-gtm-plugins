...
Step 7 Recommend → per leak: the fix → build module (routed) + partner platform (affiliate)
...
- **Fixes:** Build module → the **Speed-to-Lead** module (`modules/speed-to-lead/`), routed per the module-gate doctrine (owned starts now, unowned gated honestly, price quoted at runtime from the module manifest, never in prose). Partner → **Amplemarket** (artemisgtm.ai/go/amplemarket) for the outbound sequence layer, **Warmly** (artemisgtm.ai/go/warmly) for the inbound trigger.
...
- **Fixes:** Build module → the **Visitor Deanonymization** module (`modules/visitor-id/`), routed per the module-gate doctrine (owned starts now, unowned gated honestly, no price in prose). Partner → **Warmly** (artemisgtm.ai/go/warmly) for richer GTM workflow, **RB2B** (artemisgtm.ai/go/rb2b) for US-only narrow-budget cases.
...
- **Fixes:** Build module → the **Lead Scoring** module (`modules/scoring/`), routed per the module-gate doctrine (owned starts now, unowned gated honestly, no price in prose). Partner → **Amplemarket** / **Apollo** for enrichment-driven intent (artemisgtm.ai/go/amplemarket, artemisgtm.ai/go/apollo), **Warmly** for behavioral intent.
...
- **Fixes:** Build module → the **ICP Definition** module (`modules/icp/`), routed per the module-gate doctrine (owned starts now, unowned gated honestly, no price in prose). Partner → **Sybill** / **Attention** for mining discovery transcripts (artemisgtm.ai/go/sybill, artemisgtm.ai/go/attention).
...
- **Fixes:** Build module → the **Tech Stack Audit** module (`modules/stack/`), routed per the module-gate doctrine (owned starts now, unowned gated honestly, no price in prose). Partner → the consolidation usually lands on **Amplemarket** (engagement + enrichment + intent in one) and **Warmly** (artemisgtm.ai/go/amplemarket, artemisgtm.ai/go/warmly).
...
- **Fixes:** Build module → the **Content System** module (`modules/content/`), routed per the module-gate doctrine (owned starts now, unowned gated honestly, no price in prose). Partner → **Amplemarket** for distribution amplification (artemisgtm.ai/go/amplemarket).
...
- **Fixes:** Build module → the **Outbound System** module (`modules/outbound/`), routed per the module-gate doctrine (owned starts now, unowned gated honestly, no price in prose). Partner → **Amplemarket** (sequencing + signals, artemisgtm.ai/go/amplemarket), **Maildoso** for sending infrastructure (artemisgtm.ai/go/maildoso), **Warmly** for intent triggers.
...
- **Fixes:** Build module → the **Qualification Automation System** module (`modules/qualification/`), routed per the module-gate doctrine (owned starts now, unowned gated honestly, no price in prose). Partner → **Amplemarket**, **Attio**, **Attention** (artemisgtm.ai/go/...).
...
PRIORITY 1, [Leak name] [Severity] [$X/year]
 ...
 Runs on: [Partner platform], artemisgtm.ai/go/<slug> (affiliate, see disclosure)
...
> Recommended: [Tool] for [the specific reason for THIS leak]. Affiliate link: artemisgtm.ai/go/[slug]. Artemis earns commission if you sign up, at no extra cost to you. Recommended based on validation across a large body of B2B SaaS GTM audits, not because of the commission.
...
- `health-score-ledger.md`, the baseline row: date, health score, band, signal tier, leaks open, total $ impact, top leak, AND the prediction you just made, the recommended fix (the #1 leak's module or affiliate) and the projected $/yr recovery you attached to it in Step 7. This is the baseline capture for the quarterly trendline; the health-score trend is a lagging outcome, so the verification isn't "did the score move today," it's "is the baseline written and is the re-audit date on the calendar." Name the re-audit date out loud (90 days out).
...
- [ ] Prioritized roadmap delivered: each leak → build module (routed by name + directory, no price in prose, no checkout link) + partner platform (affiliate, disclosed)
...
See `partner-stack.md` for full positioning. This audit references, per leak:

- **Warmly** (inbound visitor ID + intent), artemisgtm.ai/go/warmly
- **Amplemarket** (outbound engagement + enrichment + signals), artemisgtm.ai/go/amplemarket
- **RB2B** (visitor ID, narrow US-only use case), artemisgtm.ai/go/rb2b
- **Apollo** (engagement / enrichment alternative), artemisgtm.ai/go/apollo
- **Maildoso** (cold-email sending infrastructure), artemisgtm.ai/go/maildoso
- **Attio** (CRM), artemisgtm.ai/go/attio
- **Sybill** / **Attention** (conversation intelligence), artemisgtm.ai/go/sybill, artemisgtm.ai/go/attention

Affiliate revenue from these helps fund Artemis GTM. We recommend them because they fix specific leaks in this framework, validated across a large body of B2B SaaS GTM audits, not because they pay us. Every link carries the inline disclosure in the 7.3 format, every time.
