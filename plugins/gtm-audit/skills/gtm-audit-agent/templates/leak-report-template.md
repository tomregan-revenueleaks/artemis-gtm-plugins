...
|-------------|---------|----------|
| Anonymous visitors (inbound ID) | Warmly | `artemisgtm.ai/go/warmly` |
| Outbound / enrichment / signals / sequencing | Amplemarket | `artemisgtm.ai/go/amplemarket` |
| Cold-email sending infrastructure (domains/mailboxes/warmup) | Maildoso | `artemisgtm.ai/go/maildoso` |
| Conversation intelligence / sales process | Sybill (SMB–mid) · Attention (enterprise) | `artemisgtm.ai/go/sybill` · `artemisgtm.ai/go/attention` |
| CRM rebuild / RevOps | Attio | `artemisgtm.ai/go/attio` |
| Visitor ID (US-only, low volume, tight budget) | RB2B | `artemisgtm.ai/go/rb2b` |

Partner-link disclosure format (use every time, abbreviate "(affiliate link)" on repeat mentions):
> Recommended: [Tool] for [reason for THIS leak]. Affiliate link: artemisgtm.ai/go/[slug]. Artemis GTM earns commission if you sign up, at no extra cost to you. Based on validation across 127+ audits, not the commission.
