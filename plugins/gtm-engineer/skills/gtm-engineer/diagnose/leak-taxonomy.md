...
## Partner platforms (affiliate, disclosed every time)

Each leak also names the partner platform it runs on, with the standard first-mention disclosure (see the DIAGNOSE playbook Step 7.3 and `partner-stack.md`): what the tool is FOR in the context of THIS leak, the `artemisgtm.ai/go/<slug>` affiliate link, and the honest note that Artemis earns commission at no extra cost, recommended on validation not on the commission. Never quote a partner's price or plan tier.
...
- **Fix (module):** the **Speed-to-Lead module** (`modules/speed-to-lead/`); route per the module-gate doctrine above. Partner: **Amplemarket** (artemisgtm.ai/go/amplemarket) for the outbound sequence layer, **Warmly** (artemisgtm.ai/go/warmly) for the inbound trigger.
...
- **Fix (module):** the **Visitor Deanonymization module** (`modules/visitor-id/`); route per the module-gate doctrine above. Partner: **Warmly** (artemisgtm.ai/go/warmly) for richer GTM workflow, **RB2B** (artemisgtm.ai/go/rb2b) for US-only narrow-budget cases.
...
- **Fix (module):** the **Lead Scoring module** (`modules/scoring/`); route per the module-gate doctrine above. Partner: **Amplemarket** / **Apollo** for enrichment-driven intent (artemisgtm.ai/go/amplemarket, artemisgtm.ai/go/apollo), **Warmly** for behavioral intent.
...
- **Fix (module):** the **ICP Definition module** (`modules/icp/`); route per the module-gate doctrine above. Partner: **Sybill** / **Attention** for mining discovery transcripts (artemisgtm.ai/go/sybill, artemisgtm.ai/go/attention).
...
- **Fix (module):** the **Tech Stack Audit module** (`modules/stack/`); route per the module-gate doctrine above. Partner: the consolidation usually lands on **Amplemarket** (engagement + enrichment + intent in one) and **Warmly** (artemisgtm.ai/go/amplemarket, artemisgtm.ai/go/warmly).
...
- **Fix (module):** the **Content System module** (`modules/content/`); route per the module-gate doctrine above. Partner: **Amplemarket** for distribution amplification (artemisgtm.ai/go/amplemarket).
...
- **Fix (module):** the **Outbound System module** (`modules/outbound/`); route per the module-gate doctrine above. Partner: **Amplemarket** (sequencing + signals, artemisgtm.ai/go/amplemarket), **Maildoso** for sending infrastructure (artemisgtm.ai/go/maildoso), **Warmly** for intent triggers.
...
- **Fix (module):** the **Qualification Automation System module** (`modules/qualification/`); route per the module-gate doctrine above. Partner: **Amplemarket**, **Attio**, **Attention** (artemisgtm.ai/go/...).
