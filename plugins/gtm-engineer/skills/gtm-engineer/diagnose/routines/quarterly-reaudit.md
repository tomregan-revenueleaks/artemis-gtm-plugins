...
Before you re-measure anything, read the prior prediction off the last health-score-ledger.md row: the recommended fix (which leak, which module or affiliate) and the projected $/yr recovery the previous audit attached to it. Hold that prediction in mind, it is the call you are about to grade.
...
6. PREDICTION CALIBRATION (grade last quarter's call)
 ...
 - Did the buyer act on it? (Did they unlock/build the recommended module or run the affiliate fix?) Did the metric actually move? Read projected versus actual recovery: did the leak's $ impact shrink by roughly what you projected, more, less, or not at all?
