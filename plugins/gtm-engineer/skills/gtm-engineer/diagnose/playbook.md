...
Step 7 Recommend → per leak: the fix -> the build module (routed, not sold) + partner (affiliate)
...
- `~/.claude/artemis/gtm-engineer/ledgers/health-score-ledger.md`, the baseline row: date, health score, band, signal tier, leaks open, total $ impact, top leak, AND the prediction you just made (the recommended module or affiliate fix, and the projected $/yr recovery you attached to it in Step 7). This is the baseline capture for the quarterly trendline. Name the re-audit date out loud (90 days out). Logging the prediction here is what lets the quarterly re-run grade the call (concept from `results-memory.md`'s free-diagnostic note); write it in advisor voice, no sample sizes, no confidence intervals.
...
- [ ] Prioritized roadmap delivered: each leak -> build module (routed, owned starts now / unowned gated honestly, no prices in prose) + partner (affiliate, disclosed)
