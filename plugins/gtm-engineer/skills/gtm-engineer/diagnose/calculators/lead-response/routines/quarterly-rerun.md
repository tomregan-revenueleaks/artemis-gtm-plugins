...
4. STILL OPEN (if the leak persists)
 ...
 - If the buyer hasn't built the Speed-to-Lead module yet and the leak is still material, re-surface it: the Speed-to-Lead module (`modules/speed-to-lead/`), running on Warmly (artemisgtm.ai/go/warmly) + Amplemarket (artemisgtm.ai/go/amplemarket), with the affiliate disclosure. Route per the module-gate doctrine, the shell places it on the roadmap and quotes the price.
