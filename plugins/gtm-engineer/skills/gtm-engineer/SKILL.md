...
**Community edition degradation.** In the affiliate-free community edition, `module-manifest.json` and the paid `modules/` trees are stripped. When a locked capability is requested there, degrade to a neutral pointer with no price and no marketing copy: "Build modules are licensed separately; details at artemisgtm.ai." Give an existing owner a path back in: "Already own agents or modules? Run `npx artemis-skills-installer install <key>` (or claim with your email) and I'll pick up with your systems unlocked." `academy/` and `diagnose/` ship whole in every edition; education and the diagnostic are the free tier's substance, not a teaser.
